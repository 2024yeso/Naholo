...
CREATE TABLE `Where` (
    `WHERE_ID` VARCHAR(255) NOT NULL PRIMARY KEY, -- 수정: VARCHAR로 변경
    `WHERE_NAME` VARCHAR(255) NOT NULL,
    `WHERE_LOCATE` VARCHAR(255) NOT NULL,
    `WHERE_RATE` FLOAT(53) NOT NULL DEFAULT 0.0, -- 기본값 추가
    `WHERE_TYPE` VARCHAR(255) NOT NULL,
    `WHERE_IMAGE` longtext NOT NULL,
...
CREATE TABLE `LIKES` (
    ...
    `USER_ID` VARCHAR(255) NOT NULL,
    `WHERE_ID` VARCHAR(255) NOT NULL, -- 수정: VARCHAR로 변경
    FOREIGN KEY (`USER_ID`) REFERENCES `Users`(`USER_ID`) ON DELETE CASCADE,
...
-- journal_post 테이블 수정: 기존 테이블에 태그 필드 추가
ALTER TABLE journal_post
ADD COLUMN 혼캎 BOOLEAN DEFAULT FALSE,
ADD COLUMN 혼영 BOOLEAN DEFAULT FALSE,
ADD COLUMN 혼놀 BOOLEAN DEFAULT FALSE,
ADD COLUMN 혼밥 BOOLEAN DEFAULT FALSE,
ADD COLUMN 혼박 BOOLEAN DEFAULT FALSE,
ADD COLUMN 혼술 BOOLEAN DEFAULT FALSE,
ADD COLUMN 기타 BOOLEAN DEFAULT FALSE;

CREATE TABLE `Post_like` (
    `POST_LIKE_ID` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    `POST_ID` BIGINT NOT NULL,
    `USER_ID` VARCHAR(255) NOT NULL,
    FOREIGN KEY (`POST_ID`) REFERENCES `Journal_post`(`POST_ID`) ON DELETE CASCADE,
    FOREIGN KEY (`USER_ID`) REFERENCES `Users`(`USER_ID`) ON DELETE CASCADE
...
-- journal_image 테이블 생성: 이미지 데이터를 저장
CREATE TABLE journal_image (
    POST_ID BIGINT NOT NULL,         -- journal_post의 POST_ID를 참조
    IMAGE_DATA LONGTEXT NOT NULL,         -- base64 인코딩된 이미지 데이터
    FOREIGN KEY (POST_ID) REFERENCES journal_post(POST_ID) ON DELETE CASCADE
);

ALTER TABLE Journal_comment 
ADD COLUMN COMMENT_CREATE DATETIME DEFAULT CURRENT_TIMESTAMP;
